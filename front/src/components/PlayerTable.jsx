export const PlayerTable = ({ players, hasResults }) => (
  <div className="table-responsive catalog-table-scroll">
    <table className="table players-table mb-0">
      ...
      <tbody>{players.map(player => <tr key={player.id}><td>{player.id}</td><td>{player.firstName} {player.lastName}</td><td>{player.birthDate || 'N/D'}</td><td>{player.nationality || 'N/D'}</td><td>{player.position || 'N/D'}</td><td>{player.currentTeam}</td><td>{player.league}</td></tr>)}</tbody>
    </table>
